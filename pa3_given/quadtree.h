  ...
  private:
    ...
    class QuadtreeNode
    {
      public:
        ...
        QuadtreeNode(Quadtree * ob);
        QuadtreeNode(QuadtreeNode const*  node , RGBAPixel const& elem);
    ...
    void buildTreeHelper(QuadtreeNode* node, PNG const& source, int x, int y, int res);
    
    RGBAPixel getPixel_helper(QuadtreeNode* node,int  x,int y,int resolution) const;
    void decompress_helper( PNG &source) const;
    
    
    void clockwiseRotate_helper(QuadtreeNode * node);
    
    
    void prune_helper(QuadtreeNode * node, int tolerance);
    bool prunable(QuadtreeNode* node1, QuadtreeNode* node, int tolerance) const;
    
    int diff_of_Color(RGBAPixel el, RGBAPixel el1) const;
    
    void pruneSize_helper(int tolerance, int & acm, QuadtreeNode * node) const;
    void prunsize_leaves_calculate_acc(int n, int & acm) const;
    
    int idealPrune_helper(int numLeaves, int node, int a) const;
    
    void copy(Quadtree const& other);
    void copy_helper(QuadtreeNode* &thisNodePrev,QuadtreeNode* &thisNode, QuadtreeNode* other);
